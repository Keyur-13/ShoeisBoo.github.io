...
if ( class_exists( 'WP_Customize_Control' ) ) {
    ...
    class Tribunal_Custom_Radio_Image_Control extends WP_Customize_Control {
...
// Radio COlor Schema
class Tribunal_Custom_Radio_Color_Schema extends WP_Customize_Control {
 ...
 * @access public
 */
class Tribunal_Customize_Section_Upsell extends WP_Customize_Section {
...
*/
class Tribunal_Repeater_Controler extends WP_Customize_Control {
    /**
     * The control type.
     *
     * @access public
    ...
    */
    ...
    public $tribunal_box_label = '';

    public $tribunal_box_add_control = '';

    public $tribunal_box_add_button = '';
    ...
    public function __construct( $manager, $id, $args = array(), $fields = array() ) {
        $this->fields = $fields;
        $this->tribunal_box_label = $args['tribunal_box_label'] ;
        $this->tribunal_box_add_control = $args['tribunal_box_add_control'];
        $this->tribunal_box_add_button = $args['tribunal_box_add_button'];
    ...
    public function render_content() {
        ...
        <ul class="tribunal-repeater-field-control-wrap">
            <?php
            $this->tribunal_get_fields();
            ?>
        </ul>
        <input type="hidden" <?php esc_attr( $this->link() ); ?> class="tribunal-repeater-collector" value="<?php echo esc_attr( $this->value() ); ?>" />
        <?php

        if( isset( $this->tribunal_box_add_button ) && $this->tribunal_box_add_button ){ ?>
            
            <button type="button" class="button tribunal-add-control-field"><?php echo esc_html( $this->tribunal_box_add_control ); ?></button>
        <?php
        }
    ...
    private function tribunal_get_fields(){
        ...
        ?>
        <li class="tribunal-repeater-field-control">

        <div class="title-rep-wrap">
            <span class="dashicons dashicons-move twp-filter-icon"></span>
            <h3 class="tribunal-repeater-field-title"><?php echo esc_html( $this->tribunal_box_label ); ?></h3>
            <span class="dashicons dashicons-arrow-down twp-filter-icon"></span>
        </div>

        <div class="tribunal-repeater-fields">
        <?php
            foreach ($fields as $key => $field) {
            $class = isset($field['class']) ? $field['class'] : '';
            ?>
            <div class="tribunal-fields tribunal-type-<?php echo esc_attr($field['type']).' '.esc_attr( $class ); ?>">
                <?php 
                    ...
                    switch ($field['type']) {
                        ...
                        case 'selector_color':
                        $options = $field['options'];
                            echo '<div class="tribunal-type-radio">';
                        ...
                        case 'upload':
                                ...
                                esc_html_e('No image selected', 'tribunal');
                                echo '</div>';
                                echo '<div class="thumbnail thumbnail-image">';
                                echo $image;
                                echo '</div>';
                                echo '<div class="actions clearfix">';
                                echo '<button type="button" class="button twp-img-delete-button align-left">'.esc_html__('Remove', 'tribunal').'</button>';
                                echo '<button type="button" class="button twp-img-upload-button alignright">'.esc_html__('Select Image', 'tribunal').'</button>';
            ...
            <div class="clearfix tribunal-repeater-footer">
                <div class="alignright">
                <?php if( isset( $this->tribunal_box_add_button ) && $this->tribunal_box_add_button ){ ?>
                    <a class="tribunal-repeater-field-remove" href="#remove"><?php esc_html_e('Delete', 'tribunal') ?>|</a> 
                <?php } ?>
                <a class="tribunal-repeater-field-close" href="#close"><?php esc_html_e('Close', 'tribunal') ?></a>
                </div>
            </div>
        </div>
        </li>
...
if ( class_exists( 'WP_Customize_Control' ) ) {
    ...
    // Repeator Info
    class Tribunal_Info_Notiece_Control extends WP_Customize_Control {
            ...
            <span class="customize-control-title">
                ...
                <div class="twp-info-icon">
                    <div class="icon-notice-wrap">
                        <span class="dashicons dashicons-move twp-filter-icon"></span>
                        <p><?php esc_html_e( 'Click hold and drag to re-order.', 'tribunal' ); ?></p>
                    </div>
                    <div class="icon-notice-wrap">
                        <span class="dashicons dashicons-arrow-down twp-filter-icon"></span>
                        <p><?php esc_html_e( 'Click to expand settings.', 'tribunal' ); ?></p>
                    </div>
                </div>
            </span>
...
if ( class_exists( 'WP_Customize_Control' ) ) {
    ...
    // Repeator Info
    class Tribunal_Premium_Notiece_Control extends WP_Customize_Control {
            ...
            <span class="customize-control-title">
                <div class="theme-info-icon">
                    <div class="icon-notice-wrap">
                        ...
                        <span><?php echo esc_html__('More ','tribunal'). esc_html( $this->label ). esc_html__(' Available on Premium Version.', 'tribunal' ); ?></span>
                    </div>
                </div>
            </span>
